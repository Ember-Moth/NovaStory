...
export function ActionErrorBubble({
  ...
  return createPortal(
    <div
      role="alert"
      className={`border-border bg-editor-background fixed z-50 flex max-w-[min(24rem,calc(100vw-1rem))] items-start gap-2 rounded-md border px-3 py-2 shadow-lg ${textClass} text-accent-foreground`}
      style={{
        top: position.top,
    ...
    >
      ...
      <button
        ...
        onClick={onDismiss}
        aria-label="关闭"
        className="text-foreground-muted hover:bg-button-hover-background hover:text-foreground shrink-0 rounded p-0.5 transition"
      >
